...
const ISLANDS := [
	...
	{"label": "Island 5: Montréal", "scene": "res://scenes/islands/montreal.tscn",
		"spawn": Vector3(0, 1.2, 42), "track": "montreal", "display": "Montréal"},
]
...
func _travel_to_island(isl: Dictionary) -> void:
	...
	if isl.track in ["eaton", "calgary", "winnipeg", "montreal"] and not GameState.get_flag("island1_complete"):
	...
	if isl.track == "montreal" and not GameState.get_flag("island4_complete"):
