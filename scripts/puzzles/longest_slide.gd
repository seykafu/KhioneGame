...
func _burst_cellar(player: Node3D, oreo: Node3D, cam: Camera3D, spray: Node3D) -> void:
	...
	beat.tween_callback(func() -> void:
		...
		_riding = false
		_flash("The crescent sleeps under brand-new tracks. East, a mountain wears its autumn over a city, and a cross stands dark on top. (travel from the pause menu)", 6.0))
