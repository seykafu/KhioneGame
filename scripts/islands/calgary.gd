extends Node3D
## Island 3 · The Calgary Crescent — Oreo's hometown.
## A snowy crescent of bungalows under a foothill ridge, a frozen playground,
## a backyard rink, woodsmoke, magpies, and the chinook: a warm gust that
## rolls through every couple of minutes, golds the light, and softens the
## snow. Riddles land in later passes; this is the living shell.
...
# --- sky: snowfall, magpies, the chinook ---
...
func _build_chinook() -> void:
	# The chinook arch: a long cloud shelf over the western ridge with clear
	# gold sky beneath. Every couple of minutes the warm wind rolls through.
	for i in 9:
	...
	timer.timeout.connect(_chinook_gust)
	add_child(timer)

func _chinook_gust() -> void:
	# Eight golden seconds: the light leans warm, the wind swells, ice drips.
	if _chinook_on or not is_inside_tree():
		return
	_chinook_on = true
	GameState.set_flag("chinook_blowing")
	...
	if sun:
		...
		_sun_tween.tween_property(sun, "light_color", Color(1.0, 0.83, 0.58), 2.5)
		_sun_tween.tween_interval(6.0)
		_sun_tween.tween_property(sun, "light_color", Color(0.92, 0.94, 1.0), 3.0)
	var hud := get_node_or_null("../HUD")
	if hud and not _visited_locations.has("_chinook_seen"):
		_visited_locations["_chinook_seen"] = true
		hud.flash_message("A warm wind rolls off the ridge. The icicles drip, once each. A chinook.", 4.5)
	var reset := create_tween()
	reset.tween_interval(12.0)
	reset.tween_callback(func() -> void:
		_chinook_on = false
		GameState.set_flag("chinook_blowing", false))
